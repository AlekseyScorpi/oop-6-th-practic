...
public class Main {
    ...
    public static String clearTrashFromString(String str){ //чистим весь мусор из строки(в том числе и пробелы)
    ...
    public static String nicoCipher(String message, String key){
        ...
        char[] keyChar = key.toCharArray(); //парсим сам ключ на символы
        int min, index;
        ...
        for (int i = 0; i < words.length; i++){
            for (int j = i * key.length(); j < (i + 1) * key.length(); j++){//по сути шаг это длина ключа(слова)
                if (j < message.length()){
    ...
    public static String pilish_string(String str){
        ...
        if (tempNum > 0){//если строка закончилась, но до пи ещё надо добаить
            ...
            result.append(tempWord).append(" "); //вроде как докидываем если остался какой-то мусор(значит он не мусор)
        }
